...
public class BookingController {
    ...
    public ApiResponse<BookingResponse> confirmBooking(@RequestBody BookingRequest request, HttpServletRequest httpRequest){
        return ApiResponse.<BookingResponse>builder()
                .result(bookingService.booking(request, httpRequest))
                .build();
    }
    ...
    public ApiResponse<PageResponse<BookingResponse>> getHistoryBooking(
            @RequestParam(value = "page", required = false, defaultValue = "1") int page,
            @RequestParam(value = "size", required = false, defaultValue = "10") int size){
        return ApiResponse.<PageResponse<BookingResponse>>builder()
                .result(bookingService.getHistoryBookingByUser(page,size))
                .build();
    }
